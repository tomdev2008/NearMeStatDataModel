...
public class IpWhiteAction extends ActionSupport {
	...
	public String execute(){
		...
		if("post".equalsIgnoreCase(request.getMethod())){
			...
			if(!StringUtil.isNullOrEmpty(ipStart)){
				...
				if("".equals(resultMsg)){
					...
					int result = service.addIPWhiteFilter(filter);
					if(result > 0){
						resultMsg += "添加成功";
		...
		if(deleteFilter != null){
			...
			if(id > 0){
				int result = service.deleteWhiteFilter(id);
				if(result > 0){
					resultMsg += "删除成功";
					//CachedItems.removeCache("IP_BLACK_LIST");
				}else{
		...
		ipList=service.getWhiteFilterList();
