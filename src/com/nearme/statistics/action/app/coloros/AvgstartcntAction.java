 ...
 */
public class AvgstartcntAction extends ActionSupport {
	private static final long serialVersionUID = -4656688923848692953L;

	public static final String TAB_RESULT = "dm_hive_cos_avgstartcnt";// 结果表
	private final String TAG = "人均启动次数排行";
	private ColorosForm form;
	private String errorinfo;// 错误信息
	private List<COSAvgstartcntrankEntity> avgstartcntrankList;
	private List<COSAvgstartcntrankEntity> avgstartrankweekList; //人均周排行
	...
	public String init() {
		...
		avgstartrankweekList = service.getAvgstartrankweekList(dto);
		
		queryjob();

		return "startavgrank_week";
	}
		...
		if (taskcnt < Constants.HIVE_TASK_LIMIT) {
			...
			if (isSameQuery(hdto)) {
				...
				Thread thread = new Thread(new Runnable() {
					public void run() {
						service.queryAndInsertAvgstartcntrankList(fbdto, fhdto);
						// 5.完成任务 修改状态
	...
	public String queryresult() {
		...
		if (null == avgstartcntrankList || avgstartcntrankList.size() <= 0) {
			errorinfo = "查询结果为空";
		}

		return "avgstartcntrank_jobresult";
	}
	...
	private void doqueryresult() {
		...
		avgstartcntrankList = service.doGetAvgstartcntrankList(hdto);
	}
	...
	public String deleteresult() {
		...
		service.deleteAvgstartcntrank(hdto);
	...
	public String querySQL() {
		...
		hivesqlList = service.getHqlAvgstartcntrank(dto);
	...
	public String exportresult() {
		...
		try {
			...
			for (COSAvgstartcntrankEntity entity : avgstartcntrankList) {
				eu.writeLine(new String[] {
						...
						String.valueOf(entity.getAvgstartcnt()) });
			}
	...
	public List<COSAvgstartcntrankEntity> getAvgstartcntrankList() {
		return avgstartcntrankList;
	}

	public void setAvgstartcntrankList(
			List<COSAvgstartcntrankEntity> avgstartcntrankList) {
		this.avgstartcntrankList = avgstartcntrankList;
	}
	...
	public List<COSAvgstartcntrankEntity> getAvgstartrankweekList() {
		return avgstartrankweekList;
	}

	public void setAvgstartrankweekList(
			List<COSAvgstartcntrankEntity> avgstartrankweekList) {
		this.avgstartrankweekList = avgstartrankweekList;
	}
}
