 ...
 */
public class InstallrankAction extends ActionSupport {
	private static final long serialVersionUID = -4656688923848692953L;

	public static final String TAB_RESULT = "dm_hive_cos_install_rank";// 结果表
	private final String TAG = "安装排行";
	private ColorosForm form;
	private String errorinfo;// 错误信息
	private List<COSInstallrankEntity> installrankList;
	private List<COSInstallrankEntity> installrankweekList;//安装排行周数据
	...
	public String init() {
		...
		installrankweekList = service.getInstallrankweekList(dto);
		
		queryjob();

		return "installrank_week";
	}
		...
		if (taskcnt < Constants.HIVE_TASK_LIMIT) {
			...
			if (isSameQuery(hdto)) {
				...
				Thread thread = new Thread(new Runnable() {
					public void run() {
						service.queryAndInsertInstallrankList(fbdto, fhdto);
						// 5.完成任务 修改状态
	...
	public String queryresult() {
		...
		if (null == installrankList || installrankList.size() <= 0) {
			errorinfo = "查询结果为空";
		}

		return "installrank_jobresult";
	}
	...
	private void doqueryresult() {
		...
		installrankList = service.doGetInstallrankList(hdto);
	}
	...
	public String deleteresult() {
		...
		service.deleteInstallrank(hdto);
	...
	public String querySQL() {
		...
		hivesqlList = service.getHqlInstallrank(dto);
	...
	public String exportresult() {
		...
		try {
			...
			eu.writeLine(new String[] { "日期", "排名", "应用", "安装次数", "升级次数" });

			// 设置信息体
			eu.setFont(ExcelUtil.getBodyFont());
			for (COSInstallrankEntity entity : installrankList) {
				eu.writeLine(new String[] {
						...
						String.valueOf(entity.getUpgradecnt()) });
			}
	...
	public List<COSInstallrankEntity> getInstallrankList() {
		return installrankList;
	}

	public void setInstallrankList(List<COSInstallrankEntity> installrankList) {
		this.installrankList = installrankList;
	}
	...
	public List<COSInstallrankEntity> getInstallrankweekList() {
		return installrankweekList;
	}

	public void setInstallrankweekList(
			List<COSInstallrankEntity> installrankweekList) {
		this.installrankweekList = installrankweekList;
	}
}
