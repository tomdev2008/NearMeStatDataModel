 ...
 * @since 1.0, 2013-9-4
 */
public class ProductreportmonthlyAction extends ActionSupport {
	private static final long serialVersionUID = -1940603784500226768L;

	private GameForm form;
	private List<ProductEntity> productList;// 产品列表
	private List<ProductreportmonthlyEntity> productreportmonthlyList;
	...
	public String init() {
		...
		String startDate = DateUtil.getDateOfXmonthAgo(4);
		...
		productreportmonthlyList = service.getProductreportmonthlyList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, productreportmonthlyList);
		
		LogUtil.log(dto, "产品月报");
	...
	public String export() {
		...
		List<ProductreportmonthlyEntity> list = (List<ProductreportmonthlyEntity>) obj;
		...
		try {
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				ProductreportmonthlyEntity entity = list.get(i);
	...
	public List<ProductreportmonthlyEntity> getProductreportmonthlyList() {
		return productreportmonthlyList;
	}

	public void setProductreportmonthlyList(
			List<ProductreportmonthlyEntity> productreportmonthlyList) {
		this.productreportmonthlyList = productreportmonthlyList;
	}
