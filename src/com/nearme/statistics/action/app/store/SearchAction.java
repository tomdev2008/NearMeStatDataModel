...
public class SearchAction extends ActionSupport {
	...
	public String init() {
		...
		searchList = service.getSearchList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, searchList);
	...
	public String export() {
		...
		List<SearchEntity> list = (List<SearchEntity>) obj;

		HttpServletResponse response = ServletActionContext.getResponse();
		ExcelUtil eu = new ExcelUtil("搜索报表_" + DateUtil.getToday());

		try {
			eu.createSheets(1, new String[] { "搜索报表" });

			// 设置标题
			eu.setFont(ExcelUtil.getTitleFont());
			eu.writeLine(new String[] { "日期", "搜索下载总量", "搜索总量", "搜索下载总用户数",
					"搜索总用户数", "搜索未下载用户数", "搜索次数", "搜索产生下载次数", "搜索未产生下载次数" });
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				...
				eu.writeLine(new String[] {
						...
						String.valueOf(entity.getTotal()),
						String.valueOf(entity.getDownTotalUser()),
						String.valueOf(entity.getTotalUser()),
						String.valueOf(entity.getNotDownUser()),
						String.valueOf(entity.getSearchPhbCnt()),
						String.valueOf(entity.getSearchBaiduCnt()) });
			}
			eu.endWrite();
			eu.export(response);
		} catch (Exception e) {
