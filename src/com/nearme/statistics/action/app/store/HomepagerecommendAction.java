 ...
 * @since 1.0, 2013-7-5
 */
public class HomepagerecommendAction extends ActionSupport {
	private static final long serialVersionUID = 9038018083239546674L;

	private StoreForm form;
	private List<HomepagerecommendEntity> homepagerecommendList;
	...
	public String init() {
		...
		homepagerecommendList = service.getHomepagerecommendList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, homepagerecommendList);
		
		LogUtil.log(dto, "首页推荐");
	...
	public String export() {
		...
		List<HomepagerecommendEntity> list = (List<HomepagerecommendEntity>) obj;

		HttpServletResponse response = ServletActionContext.getResponse();
		ExcelUtil eu = new ExcelUtil("首页推荐报表_" + DateUtil.getToday());

		try {
			eu.createSheets(1, new String[] { "首页推荐报表" });
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				HomepagerecommendEntity entity = list.get(i);
	...
	public List<HomepagerecommendEntity> getHomepagerecommendList() {
		return homepagerecommendList;
	}

	public void setHomepagerecommendList(
			List<HomepagerecommendEntity> homepagerecommendList) {
		this.homepagerecommendList = homepagerecommendList;
	}
}
