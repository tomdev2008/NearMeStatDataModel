 ...
 * @since 1.0, 2013-8-6
 */
public class PoprecentAction extends ActionSupport {
	private static final long serialVersionUID = 9038018083239546674L;

	private StoreForm form;
	private List<PoprecentEntity> poprecentList;
	...
	public String init() {
		...
		poprecentList = service.getPoprecentList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, poprecentList);
		
		LogUtil.log(dto, "最近流行");
	...
	public String export() {
		...
		List<PoprecentEntity> list = (List<PoprecentEntity>) obj;

		HttpServletResponse response = ServletActionContext.getResponse();
		ExcelUtil eu = new ExcelUtil("最近流行_" + DateUtil.getToday());

		try {
			eu.createSheets(1, new String[] { "最近流行" });
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				PoprecentEntity entity = list.get(i);
	...
	public List<PoprecentEntity> getPoprecentList() {
		return poprecentList;
	}

	public void setPoprecentList(List<PoprecentEntity> poprecentList) {
		this.poprecentList = poprecentList;
	}
