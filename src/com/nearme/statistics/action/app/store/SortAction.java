...
public class SortAction extends ActionSupport {
	...
	public String init() {
		...
		sortList = service.getSortList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, sortList);
	...
	public String export() {
		...
		List<SortEntity> list = (List<SortEntity>) obj;

		HttpServletResponse response = ServletActionContext.getResponse();
		ExcelUtil eu = new ExcelUtil("分类_" + DateUtil.getToday());

		try {
			eu.createSheets(1, new String[] { "分类" });
