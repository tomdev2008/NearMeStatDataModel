 ...
 * @since 1.0, 2013-9-12
 */
public class RegisterfromAction extends ActionSupport {
	private static final long serialVersionUID = -1940603784500226768L;

	private final String TAG = "注册来源";
	private GrzxForm form;
	private List<GrzxRegisterfromEntity> registerfromList;
	...
	public String init() {
		...
		dto.setLidu(lidu);
		registerfromList = service.getRegisterfromList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, registerfromList);
	...
	public String export() {
		...
		List<GrzxRegisterfromEntity> list = (List<GrzxRegisterfromEntity>) obj;
		...
		try {
			...
			eu.writeLine(new String[] { "日期", "全平台注册帐号数", "手机注册帐号数", "邮箱注册帐号数",
					"无绑定信息注册帐号数", "游客帐号注册数", "第三方帐号注册数" });

			// 设置信息体
			eu.setFont(ExcelUtil.getBodyFont());
			for (int i = list.size() - 1; i >= 0; i--) {
				GrzxRegisterfromEntity entity = list.get(i);
				eu.writeLine(new String[] {
						DateUtil.formatDate2Short(entity.getStatDate()),
						String.valueOf(entity.getAllreg()),
						String.valueOf(entity.getMobilereg()),
						String.valueOf(entity.getEmailreg()),
						String.valueOf(entity.getNobandreg()),
						String.valueOf(entity.getVisitreg()),
						String.valueOf(entity.getThirdreg()) });
			}
			eu.endWrite();
			eu.export(response);
		} catch (Exception e) {
	...
	public List<GrzxRegisterfromEntity> getRegisterfromList() {
		return registerfromList;
	}

	public void setRegisterfromList(
			List<GrzxRegisterfromEntity> registerfromList) {
		this.registerfromList = registerfromList;
	}
