...
import com.nearme.statistics.tags.ModelSelectTag;
 ...
 * @since 1.0, 2013-8-10
 */
public class ModuledayAction extends ActionSupport {
	private static final long serialVersionUID = 3344613703440222126L;

	private BaseForm form;
	private List<ModuledayEntity> moduledayList;
	...
	public String init() {
		...
		// 获取第一个机型
		List<ModelEntity> modellist = ModelSelectTag.getModels();
		String model = modellist.get(0).getModel();
		...
		dto.setModel(model);
		moduledayList = service.getModuledayList(dto);
		ServletActionContext.getRequest().getSession().setAttribute(
				Constants.NEARME_SESSION_REPROT_DATA, moduledayList);
		
		LogUtil.log(dto, "终端日明细");//log记录查询
	...
	public List<ModuledayEntity> getModuledayList() {
		return moduledayList;
	}

	public void setModuledayList(List<ModuledayEntity> moduledayList) {
		this.moduledayList = moduledayList;
	}
}
