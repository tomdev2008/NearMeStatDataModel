...
(function($) {
	...
	$.msgbox = function(opts) {
		...
		this.mainWrap = $('<div />').css({
			...
			'top' : ($(window).height() - this.opts.height) / 2 > 500 ? 100 : 100 + 'px'
		...
		domready: function(){
			...
			switch(this.opts.type){
				...
				case 'input':
					...
					var str='<textarea id="reflect_info" class="suggest_textarea"></textarea><br />';
					str+='<label class="suggest_label">反馈人：</label>';
					str+='<input id="reflect_name" class="suggest_input" type="text" /><br />';
					var inputbox=$(str);
					var okbutton = $('<input class="suggest_btn" type="button" value="提交" />').click(function(){
						//_this.setValue( inputbox.val() );
						suggestSubmit();
						_this.close();
					});
					var div = $('<div />').css('padding','10px').append(inputbox).append(okbutton);
