					...
					<tr>
						...
						<td align="center">
							...
							<input type="button" value="删除" class="btn-style"
								onclick="if(confirm('确定删除  ${channelName }  渠道?')){btnDeleteClick('${form.systemID }','${channelID}');}" />
						</td>
					</tr>
				</s:iterator>
			</table>
...
// 删除
function btnDeleteClick(systemID, channelID) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		...
		success : function(result) {
			window.location.href = "setting_channelinfo_init.do?form.systemID="+systemID;
		},
		error : function(data) {
			alert("error:" + data.responseText);
		}
...
function addChannelinfo() {
	...
	$.ajax( {
		...
		success : function(result) {
			window.location.href = "setting_channelinfo_init.do?form.systemID="
					+ systemID;
		},
		error : function(data) {
			alert("error:" + data.responseText);
