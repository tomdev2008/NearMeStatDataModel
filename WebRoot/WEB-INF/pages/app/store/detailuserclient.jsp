				...
				<tr>
					<td>
						...
						<s:submit name="query" value="查询"
							action="store_detailuserclient_query">
						</s:submit>
						<s:submit name="export" value="导出"
							action="store_detailuserclient_export">
						</s:submit>
					</td>
				</tr>
			</table>
			...
			<table id="dataSourceTbl" class="data-load" border="1"
				...
				<tr class="data-head">
					...
					<th onclick="showTblChart(1)" onMouseOver="this.bgColor='#FF9900'"
						onMouseOut="this.bgColor=''" style="cursor: hand" title="客户端启动用户数">
						客户端启动用户数
					</th>
				...
				<s:iterator value="detailuserclientList">
					<tr>
						<td>
...
function changeLidu() {
	...
	if (document.getElementById("lidu").value == "DAILY") {
		...
		endtime = new Date().dateAdd('w', -1).format("yyyy-MM-dd");
	} else {
		starttime = new Date().dateAdd('m', -4).format("yyyy-MM-dd");
		endtime = new Date().dateAdd('m', -1).format("yyyy-MM-dd");
	}
