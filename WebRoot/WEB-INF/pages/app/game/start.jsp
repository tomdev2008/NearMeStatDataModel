			...
			<table>
				<tr>
					<td>
						...
						<s:submit name="export" value="导出" action="game_start_export">
						</s:submit>
					</td>
				</tr>
			</table>
		</s:form>
		<div class="data-load" id="div_start">
		    <s:include value="start_start.jsp" />
		</div>
...
function submitQuery() {
	...
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'game_start_query.do',
		data : $('#form').serialize() + '&form.productIDs=' + ids,
		success : function(result) {
			$("#div_start").html(result);
		},
		error : function(data) {
			alert("error:" + data.responseText);
		}
