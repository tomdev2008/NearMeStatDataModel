			...
			<table>
				<tr>
					<td>
						...
						<s:submit name="export" value="导出" action="game_remain_export">
						</s:submit>
					</td>
				</tr>
			</table>
		</s:form>
		<div class="data-load" id="div_remain">
			<s:include value="remain_query.jsp" />
		</div>
...
function showTblChart(cloumn) {
	var fcfCahrt = new FusionCharts("../../FCF/ScrollLine2D.swf", "fcfCahrt",
			"1100", "350", "0", "1");
	chartText = table2FCFSingleLineChart("dataSourceTbl2", cloumn, null);
...
function submitQuery() {
	...
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'game_remain_query.do',
		data : $('#form').serialize() + '&form.productid=' + ids,
		success : function(result) {
			$("#div_remain").html(result);
		},
		error : function(data) {
			alert("error:" + data.responseText);
		}
