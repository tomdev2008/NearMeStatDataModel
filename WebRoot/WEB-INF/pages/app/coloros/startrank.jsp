...
function suggestSubmit() {
	...
	<body class="context">
		...
		<div class="data-load" id="div_jobresult">
			<s:include value="startrank_jobresult.jsp" />
		</div>
		<script type="text/javascript">
...
function jumpPage() {
	...
	window.location.href = "coloros_startrank_init.do?form.systemID="+systemID;
}
...
function submitQuery() {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'coloros_startrank_query.do',
		data : $('#form').serialize(),
		success : function(result) {
...
function queryJob() {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'coloros_startrank_queryjob.do',
		data : $('#form').serialize(),
		success : function(result) {
			$("#div_listjob").html(result);
		},
...
function queryresult(jobID, resulttable, weidu, lidu) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'coloros_startrank_queryresult.do',
...
function deleteresult(jobID, resulttable, weidu, lidu) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'coloros_startrank_deleteresult.do',
		data : '&form.jobID=' + jobID
		     + '&form.resulttable=' + resulttable
		     + '&form.weidu=' + weidu
		     + '&form.lidu=' + lidu,
...
function querySQL(){
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'coloros_startrank_querySQL.do',
		data : $('#form').serialize(),
		success : function(result) {
			$("#div_listjob").html(result);
		},
...
function exportresult(jobID,resulttable,model) {
	// 指定action
	document.getElementById('form_export').action = "coloros_startrank_exportresult.do";
