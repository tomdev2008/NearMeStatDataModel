...
function suggestSubmit() {
	...
	<body class="context">
			...
			<table>
				<tr>
					<td>
						...
						模块
						<select name="form.groupname" id="form.groupname">
							<s:iterator value="moduleList">
								<option value="${groupname}">${groupname}</option>
							</s:iterator>
						</select>
						位置数
						<input type="text" id="form.position" name="form.position" value="${form.position}" onkeyup="checkPosition();" />
		...
		<div class="data-load" id="div_jobresult">
			<s:include value="module_position_jobresult.jsp" />
		</div>
		<script type="text/javascript">
function checkPosition(){
	var tt = document.getElementById("form.position");
	if (isNaN(tt.value)){alert("非法字符！");tt.value="20";}
	if(tt.value<1 || tt.value>200){alert("取值范围1~200,超出范围！");tt.value="20";}
}
...
function submitQuery() {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_moduleposition_query.do',
		data : $('#form').serialize(),
		success : function(result) {
...
function queryJob() {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_moduleposition_queryjob.do',
		data : $('#form').serialize(),
		success : function(result) {
			$("#div_listjob").html(result);
		},
...
function queryresult(jobID, resulttable, weidu, lidu) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_moduleposition_queryresult.do',
...
function deleteresult(jobID, resulttable, weidu, lidu) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_moduleposition_deleteresult.do',
		data : '&form.jobID=' + jobID 
		     + '&form.resulttable=' + resulttable
		     + '&form.weidu=' + weidu
		     + '&form.lidu=' + lidu,
...
function querySQL(){
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_moduleposition_querySQL.do',
		data : $('#form').serialize(),
		success : function(result) {
			$("#div_listjob").html(result);
		},
...
function exportresult(jobID,resulttable,weidu,lidu) {
	// 指定action
	document.getElementById('form_export').action = "common_moduleposition_exportresult.do";
