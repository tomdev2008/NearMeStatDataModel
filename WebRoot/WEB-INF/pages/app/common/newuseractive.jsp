	...
	<body class="context">
		...
		<div class="div_help_content" id="div_help_content">
			...
			<div class="body">
			    ...
			    2.查询为选择一段时间新用户样本，查询该样本在接下来的第一个月以及第一周的表现情况;<br>
		...
		<div class="data-load" id="div_jobresult">
			<s:include value="newuseractive_jobresult.jsp" />
		</div>
		<script type="text/javascript">
function showTblChartMonth(cloumn) {
	var fcfCahrt = new FusionCharts("../../FCF/ScrollLine2D.swf", "fcfCahrt",
			"1100", "350", "0", "1");
	chartText = table2FCFSingleLineChart("dataSourceTbl_month", cloumn, "end-2-front");
	fcfCahrt.setDataXML(chartText);
	fcfCahrt.render("chartDiv");
}

function showTblChartWeek(cloumn) {
	var fcfCahrt = new FusionCharts("../../FCF/ScrollLine2D.swf", "fcfCahrt",
			"1100", "350", "0", "1");
	chartText = table2FCFSingleLineChart("dataSourceTbl_week", cloumn, "end-2-front");
	fcfCahrt.setDataXML(chartText);
	fcfCahrt.render("chartDiv");
}
...
function submitQuery() {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_newuseractive_query.do',
		data : $('#form').serialize(),
		success : function(result) {
...
function queryJob() {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_newuseractive_queryjob.do',
		data : $('#form').serialize(),
		success : function(result) {
			$("#div_listjob").html(result);
		},
...
function queryresult(jobID, resulttable, weidu, lidu) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_newuseractive_queryresult.do',
...
function deleteresult(jobID, resulttable, weidu, lidu) {
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_newuseractive_deleteresult.do',
		data : '&form.jobID=' + jobID 
		     + '&form.resulttable=' + resulttable
		     + '&form.weidu=' + weidu
		     + '&form.lidu=' + lidu,
...
function querySQL(){
	$.ajax( {
		type : "POST",
		dataType : "html",
		url : 'common_newuseractive_querySQL.do',
		data : $('#form').serialize(),
		success : function(result) {
			$("#div_listjob").html(result);
		},
...
function exportresult(jobID,resulttable,weidu,lidu) {
	// 指定action
	document.getElementById('form_export').action = "common_newuseractive_exportresult.do";
